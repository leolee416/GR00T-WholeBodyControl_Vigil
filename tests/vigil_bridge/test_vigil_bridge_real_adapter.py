...
def test_real_startup_reference_enters_streamed_mode_without_planner(
    ...
    class FakePublisher:
        ...
        def send_command(
            ...
            events.append(
                f"command:{int(start)}:{int(stop)}:{int(planner)}:{int(pause)}:{int(hold)}"
            )
    ...
    assert health["ok"] is True
    assert events == ["command:1:0:0:0:1"]
